...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CGFloat x = (self.scrollDirection == UICollectionViewScrollDirectionHorizontal) ? (indexPath.item * (self.spacing + self.itemSize.width)) + extra : (0.5 * (self.collectionView.bounds.size.width - self.itemSize.width));

    // y坐标 水平方向 = (collectionView - item宽度) / 2.0 (居中显示) ; 垂直方向 = item总数 * (item高度 + space)
    CGFloat y = (self.scrollDirection == UICollectionViewScrollDirectionHorizontal) ? (0.5 * (self.collectionView.bounds.size.height - self.itemSize.height)) : (indexPath.item * (self.spacing + self.itemSize.height)) + extra;
...
- (NSArray *)attributesInRect:(CGRect)rect
{
    ...
    // 指定范围内的第一个cell的下标
    NSInteger preIndex = (self.scrollDirection == UICollectionViewScrollDirectionHorizontal) ? ((rect.origin.x - extra) / (self.itemSize.width + self.spacing)) : ((rect.origin.y - extra) / (self.itemSize.height + self.spacing));
    preIndex = ((preIndex < 0) ? 0 : preIndex); // 防止下标越界

    // 指定范围内的最后一个cell下标
    NSInteger latIndex = (self.scrollDirection == UICollectionViewScrollDirectionHorizontal) ? ((CGRectGetMaxX(rect) - extra) / (self.itemSize.width + self.spacing)) : ((CGRectGetMaxY(rect) - extra) / (self.itemSize.height + self.spacing));
